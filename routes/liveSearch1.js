...
//app.post('/liveSearch1', /*cors(allowCrossDomain),*/ jsonParser, function(req, res){
       ...
       function getTheDaises(searchQuery){
               /*console.log('It Entered    {{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{');
               console.log(req.body.searchQuery);
               console.log('seeing it')*/
                mongo.connect(url, function(err, db){
                      if(err)throw err;
                        var cursor = db.collection('Daises').find({$text: {$search: searchQuery}}, {score: {$meta: "textScore"}}).sort({score:{$meta:"textScore"}}).count();
