...
router.post('/', function(req, res){
       ...
       var writeToFile;
       var getFile;
       var readFileStreamAgain;
       var creatingWriteStreamAgain;
             ...
             mongo.connect(url, function(err, db){
                if(err)throw err;
                  ...
                  function gettingOnlineStatusSQL(returnedDaises){
                        //console.log(counterForDoc)
                        //console.log("lslslslslslslslslslsl")
                  ...
                  /*  function nowWritingToFile(emailStore){
                        var counter = 0;
                        for(var i=0; i < emailStore.length; i++){
                           counter++
                           justCreatedObject[emailStore[i]] = 'offline';

                           if(counter == emailStore.length){
                               creatingWriteStreamAgain = fs.createWriteStream('./public/onlineDataBase/onlineStatus.json');
                               creatingWriteStreamAgain.write(JSON.stringify(justCreatedObject));
                               creatingWriteStreamAgain.end(function(){
                    ...
                    var justCreatedObject;
                    var gottenArray;
                    function callArrayData(passedArrayStore){
                           console.log(passedArrayStore);
                           gottenArray = passedArrayStore;
                           console.log('just want to see it how it looks');
                           var secondCounter = 0;
                           var emailStore = [];
                           readFileStreamAgain = fs.createReadStream('./public/onlineDataBase/onlineStatus.json');
                           readFileStreamAgain.on('open', function(){
                               console.log('opened for reading ...............');
                           });

                           readFileStreamAgain.on('data', function(chunk){
                               justCreatedObject = JSON.parse(chunk);
                           })

                           readFileStreamAgain.on('end', function(){
                              for(var i=0; i < gottenArray.length; i++){
                                   console.log(secondCounter)
                                   console.log(gottenArray[i])
                                   if(gottenArray[i].Email == ""){
                                       continue;
                                   }
                                   secondCounter++;
                                   emailStore.push(gottenArray[i].Email.split("@")[0]);
                                   if (secondCounter == gottenArray.length) {
                                        nowWritingToFile(emailStore);
                                   }
                              }
                           })
                    }*/
